...
\section*{Sendiv}
\textit{\textbf{Captains Log:} We have arrived in orbit around Sendiv, a planet perpetually blanketed by ferocious lightning storms. Recent observations have revealed a complex underlying structure in this electrical activity.}% We are here to learn more.}% about this phenomenon.}

\textit{Currently aboard the Endeavour are civilian researchers who specialize in atmospheric physics and xenoarchaeology. Our orders are to deliver them to the planet's surface, support their research efforts, and ensure their safety.}
...
\textbf{Dr. Stevens} assembles the researchers into small teams and asks that they be distributed at regular intervals across the planet's biggest continent. Doing so will require waiting for a convenient break in the storms and then delivering each team to their destination before the opportunity passes.

These conditions will make it impossible to mount a rescue operations from the Endeavour.  Each team will be largely on their own if anything goes wrong.
...
\begin{itemize}
	...
	\item \textit{Or will you try to convince Dr. Stevens to change his plan?} \textbf{Leadership \& Negotiation} vs. \textbf{Maynard Stevens}. This is a \textit{Sensitive} challenge \textemdash{} many of the researchers are wary of ICF interference in their work.
\end{itemize}
...
\subsubsection*{A Xenoarchaeological Discovery}
This is a test
...
\subsection*{Crisis}
\begin{itemize}
	\item \textit{Will you try to defend the researchers from Namarrkon's wrath?} \textbf{Threats:} Several of the crew are caught in the storms and killed.
	\item \textit{Or will you try to convince the researchers to abandon their instruments and evacuate the planet?} \textbf{Threats:} Namarrkon's lightning prevents some teams of researchers from evacuating.
\end{itemize}
...
\subsection*{Characters}
\begin{description}
	\item[Dr. Stevens (d8):] Lead Researcher (d10), Prickly (d8), Traditional (d8), Efficient (d8).
	\item[Character Name (d?):] Roles and associated role die sizes.	
	\item[Namarrkon (d12):] Sentient Storm (d10), Ancient (d10), Sacred (d10 \textit{Sensitive}), Vengeful (d10 \textit{Grueling}), \textsc{\textbf{Distributed}} (Cannot be affected by local phenomena).
\end{description}

\subsection*{Places}
\begin{description}
	\item[Storm Farms:] Turbines, lightning rods, and batteries. Dozens of storm farms are scattered around the planet.
	\item[Governer's Mansion:] Extravagant, modern, and filled with luxuries imported from off-world.
\end{description}
